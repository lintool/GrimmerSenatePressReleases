...
html=['http://menendez.senate.gov/newsroom/index.cfm?maxrows=800&startrow=243&']
month= {}
month['01']= 'Jan'
month['02'] = 'Feb'
month['03'] = 'Mar'
...
for j in range(0, len(html)):
    ...
    for num in range(len(fr)):
        store += 'http://menendez.senate.gov/newsroom/' + fr[num] + '\n'
    fr = store.split('\n')
    fr.remove('')
    ...
    for num in range(0,len(fr)):
        ...
        mint= date[num]
        names = mint  + 'Menendez'  + str(num) + '.txt'
        files = open(names, 'w')
        files.write(stores)
        files.close()
